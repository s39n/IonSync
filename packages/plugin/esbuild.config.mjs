...
const postBuildPlugin = {
  ...
  setup(build) {
    ...
    build.onEnd(async () => {
      ...
      // Deterministic build id. Derived from the pre-stamp bundle bytes + the
      // version, NOT wall-clock time, so a clean checkout of a given commit
      // always produces byte-identical output — the community reviewer's
      // rebuild matches the released main.js (reproducible build). It still
      // changes whenever the source or version changes, so the server's
      // version_check (a string !== comparison; see handlers/versionCheck.ts)
      // continues to detect real updates. Hashing the placeholder bundle avoids
      // a chicken-and-egg with the stamp we are about to insert.
      ...
      // main.js + styles.css join them so the community reviewer's build
      // verification (which looks for a built main.js at the repo root) passes;
      // both are git-ignored at the root — they're regenerated by this build.
      const REPO_ROOT = path.join(__dirname, "..", "..") + path.sep;
      for (const file of ["manifest.json", "versions.json", "main.js", "styles.css"]) {
        if (fs.existsSync(BUILD_DIR + file)) {
